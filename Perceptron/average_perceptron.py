...
class AveragePerceptron():
    ...
    def train_perceptron(self, features, labels):
        ...
        self._average_weight_vector = np.zeros_like(self._weight_vector)
        ...
        for epoch in range(self.T):
            for index in shuffled_indices[epoch]:
                ...
                # accumulating the current weight vector into the average weight vector
                self._average_weight_vector += self._weight_vector
    ...
    def predict_labels(self, features):
        ...
        for feature_vector in features:
            predicted_labels.append(np.sign(np.dot(feature_vector, self._average_weight_vector)))
...
def main():
    ...
    a_perceptron = AveragePerceptron(learning_rate=utilities.LEARNING_RATE, T=utilities.T)
    a_perceptron.train_perceptron(train_features, train_labels)

    # displaying the learned weight vector
    learned_weights = a_perceptron.get_average_weight_vector()
    print("\nAverage Perceptron Learned Weight Vector:")
    print(learned_weights)

    # predictions 
    predictions = a_perceptron.predict_labels(test_features)
    ...
    print("\nAverage Perceptron Test Results:")
