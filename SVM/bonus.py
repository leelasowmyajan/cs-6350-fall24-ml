...
def kernel_perceptron(train_features, train_labels, test_features, test_labels):
    ...
    for gamma in utilities.LR:
        ...
        kernel_fn = lambda feature_set_1, feature_set_2: gaussian_dual_svm.gaussian_kernel(feature_set_1, feature_set_2, gamma)
        
        # train the Kernel Perceptron
        kp = KernelP(kernel_fn)
        kp.train(train_features, train_labels)

        # calculating training error
        train_predictions = kp.predict(train_features)
        train_error = np.sum(train_predictions != train_labels)
        train_error_percent = (train_error / len(train_labels)) * 100

        # calculating testing error
        test_predictions = kp.predict(test_features)
...
def main():
    ...
    kernel_perceptron(train_features, train_labels, test_features, test_labels)
    return 0
